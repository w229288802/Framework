	...
	$.fn.extend({
		...
		layoutH : function($refBox) {
			return this.each(function() {
						...
						if ($this.isTag("table")) {
							$this.removeAttr("layoutH").wrap('<div layoutH="'
									+ iLayoutH
	...
	$.fn.jBar = function(options) {
		...
		return this.each(function() {
			...
			$(op.toggleBut, bar).click(function() {
				...
				var cleft = $(op.container).cssv("left")
						- (bar.outerWidth() - sbar.outerWidth());
				var cwidth = bar.outerWidth() - sbar.outerWidth()
				...
				$(op.container).animate({
							...
							bar.animate({
										...
										$(window)
												.trigger(DWZ.eventType.resizeGrid);
									});
						});
				...
				$(op.splitBar2).each(function() {
					...
					spbar2.jDrag($.extend(options, {
								...
								stop : function() {
									...
									var move = $(this).cssv("left")
											- $(op.splitBar).cssv("left");
...
(function($) {
	$.extend($.fn, {
			...
			return this.each(function() {
				...
				$(">li", $this).each(function() {
					...
					$li.genTree({
								...
								options : op,
								level : 0,
								exp : (cnum > 1 ? (first ? op.firstExp : (last
										? op.lastExp
										: op.exp)) : op.endExp),
								coll : (cnum > 1
										? (first ? op.firstColl : (last
												? op.lastColl
												: op.coll))
										: op.endColl),
								showSub : (!$this.hasClass("collapse") && ($this
										.hasClass("expand") || (cnum > 1
										? (first ? true : false)
										: true))),
								isLast : (cnum > 1
										? (last ? true : false)
				...
				setTimeout(function() {
					...
					$("a", $this).click(function(event) {
						...
						var $li = $(this).parents("li:first"), sTarget = $li
								.attr("target");
						if (sTarget) {
							if ($("#" + sTarget, $this).size() == 0) {
								$this.prepend('<input id="' + sTarget
										+ '" type="hidden" />');
							}
			...
			return this.each(function() {
						...
						if (tree.size() > 0) {
							...
							$(">div>div:first,>div>a", node).click(function() {
								...
								var $this = isA ? $(">div>div", node)
										.eq(op.level) : $this;
								if (!isA || tree.is(":hidden")) {
									$this.toggleClass(op.exp)
											.toggleClass(op.coll);
									if (op.icon) {
										$(">div>div:last", node)
												.toggleClass(op.options.folderExp)
												.toggleClass(op.options.folderColl);
									}
								}
								(tree.is(":hidden"))
										? tree.slideDown("fast")
										: (isA ? "" : tree.slideUp("fast"));
								return false;
							});
			...
			function addSpace(level, node) {
				if (level > 0) {
					...
					if (level > 1) {
						...
						var prev = "";
						while (level > 1) {
							prev = prev + "<div class='" + next.attr("class")
									+ "'></div>";
							next = next.next();
							level--;
						}
		...
		_checkParent : function() {
			...
			var aClass = (ckboxed == ckbox ? "checked" : (ckboxed != 0
					? "indeterminate"
					: "unchecked"));
			var rClass = (ckboxed == ckbox ? "indeterminate" : (ckboxed != 0
					? "checked"
					: "indeterminate"));
...
function initUI(_box) {
	...
	if ($.fn.uploadify) {
		$(":file[uploaderOption]", $p).each(function() {
					...
					var uploaderOption = DWZ.jsonEval($this
							.attr("uploaderOption"));
	...
	$("a[target=navTab]", $p).each(function() {
		$(this).click(function(event) {
			...
			var url = unescape($this.attr("href"))
					.replaceTmById($(event.target).parents(".unitBox:first"));
			DWZ.debug(url);
			if (!url.isFinishedTm()) {
...
var navTab = {
	...
	_init : function() {
		...
		this._getTabs().each(function(iTabIndex) {
					...
					$(this).find(navTab._op.close$).unbind("click").click(
							function() {
	...
	_switchTab : function(iTabIndex) {
		...
		this._getMoreLi().removeClass("selected").eq(iTabIndex)
				.addClass("selected");
	...
	_closeOtherTab : function(index) {
		...
		if (index > 0) {
			...
			this._getPanels().not(str$).filter(":gt(0)")
					.trigger(DWZ.eventType.pageClear).remove();
	...
	_reload : function($tab, flag) {
		...
		if (flag && url) {
			...
			if ($tab.hasClass("external")) {
				...
				var args = $pagerForm.size() > 0
						? $pagerForm.serializeArray()
						: {}
	...
	openTab : function(tabid, url, options) { // if found tabid replace tab,
		...
		if (iOpenIndex >= 0) {
			...
			var span$ = $tab.attr("tabid") == this._op.mainTabId
					? "> span > span"
					: "> span";
		...
		tabs : function(options) {
			...
			function switchTab(jT, iTabIndex) {
				...
				if (!jGroup.attr("inited")) {
					jGroup.attr("inited", 1000).find("input[type=text]")
							.filter("[alt]").inputAlert();
				}
			}
	...
	$.pdialog = {
		...
		_init : function(dialog, options) {
			...
			if (isNaN(dialog.height()) || dialog.height() < height) {
				...
				$(".dialogContent", dialog).height(height
						- $(".dialogHeader", dialog).outerHeight()
		...
		resizeDialog : function(obj, dialog, target) {
			...
			if (target != "w" && target != "e") {
				...
				content.css({
							height : height
									- $(".dialogHeader", dialog).outerHeight()
									- $(".dialogFooter", dialog).outerHeight()
									- 6
						});
	...
	var sortDrag = {
		...
		drag : function(el, event) {
			var $helper = $(arguments[0]), $sortBox = $helper.data('$sortBox'), $placeholder = $helper
					.data('$placeholder');
			var $items = $sortBox.find($helper.data('op')['items'])
					.filter(':visible')
			...
			$helper.animate({
						...
						complete : function() {
							if ($helper.data('op')['replace']) { // 2个sortBox之间替换处理
								...
								$destBox = $placeholder.parents(_op.sortBoxs
										+ ":first");
		...
		_getOverSortBox : function($item, e) {
			...
			return $(_op.sortBoxs).filter(':visible').filter(function() {
				var $sortBox = $(this), sortBoxPos = $sortBox.position(), sortBoxH = $sortBox
						.height(), sortBoxW = $sortBox.width();
				return DWZ.isOver(y, x, sortBoxPos.top, sortBoxPos.left,
						sortBoxH, sortBoxW);
			});
		}
	};
	...
	$.fn.jTable = function(options) {
		return this.each(function() {
			...
			$("th", thead).each(function() {
				var $th = $(this);
				$th.html("<div class='gridCol' title='" + $th.text() + "'>"
						+ $th.html() + "</div>");
			});

			ths.each(function(i) {
				var $th = $(this), style = aStyles[i];
				$th.addClass(style[1]).hoverClass("hover").removeAttr("align")
				...
				$(">th", this).each(function(i) {
					...
					$th.mouseover(function(event) {
						...
						if ($th.outerWidth() - offset < 5) {
							...
							$th.css("cursor", $th.attr("orderField")
											? "pointer"
											: "default");
							$th.unbind("mousedown");
						}
...
function _iframeResponse(iframe, callback) {
	...
	$iframe.bind("load", function(event) {
				...
				if (iframe.src == "javascript:'%3Chtml%3E%3C/html%3E';" || // For
																			// Safari
						iframe.src == "javascript:'<html></html>';") { // For
																		// FF,
																		// IE
					return;
				}
...
(function($) {
	$.fn.extend({
		pagination : function(opts) {
			...
			return this.each(function() {
				...
				for (var i = interval.start; i < interval.end; i++) {
					pageNumFrag += setting.pageNumFrag.replaceAll("#pageNum#",
							i).replaceAll(
							"#liClass#",
							i == pc.getCurrentPage()
									? 'selected j-num'
				...
				$this.find(setting.nums$).each(function(i) {
					_bindEvent($(this), i + interval.start, pc.targetType(), pc
									.rel());
				});
		...
		orderBy : function(options) {
			...
			return this.each(function() {
						var $this = $(this).css({
							...
							var orderDirection = $this.hasClass(op.asc)
									? op.desc
									: op.asc;
							dwzPageBreak({
		...
		pagerForm : function(options) {
			...
			return this.each(function() {
				var $searchForm = $(this), $pagerForm = $(op.pagerForm$,
						op.parentBox);
				var actionUrl = $pagerForm.attr("action").replaceAll("#rel#",
						$searchForm.attr("action"));
				$pagerForm.attr("action", actionUrl);
				$searchForm.find(":input").each(function() {
					var $input = $(this), name = $input.attr("name");
					if (name
							&& (!$input.is(":checkbox,:radio") || $input
									.is(":checked"))) {
						if ($pagerForm.find(":input[name='" + name + "']").length == 0) {
							var inputFrag = frag.replaceAll("#name#", name)
									.replaceAll("#value#", $input.val());
	...
	$.extend(Pagination.prototype, {
				...
				getInterval : function() {
					...
					var start = this.getCurrentPage() > ne_half ? Math.max(Math
									.min(this.getCurrentPage() - ne_half,
											upper_limit), 0) : 0;
				...
				switch (field.type) {
					...
					case 'lookup' :
						...
						html = '<input type="hidden" name="'
								...
								+ suffix + '"/>' + '<input type="text" name="'
								+ field.name + '"' + suggestFrag
								+ ' lookupPk="' + field.lookupPk + '" size="'
								+ field.size + '" class="' + field.fieldClass
					...
					default :
						html = '<input type="' + field.type + '" name="'
								+ field.name + '" value="' + field.defaultVal
	...
	$.fn.datepicker = function(opts) {
		...
		function changeTm($input, type) {
			var ivalue = parseInt($input.val()), istart = parseInt($input
					.attr("start"))
					|| 0, iend = parseInt($input.attr("end"));
		...
		return this.each(function() {
			...
			function generateCalendar(dp) {
				...
				for (var t = 1; t <= dw.days; t++) {
					...
					var _ctrClass = (_date >= minDate && _date <= maxDate)
							...
							: 'disabled';
					if (t == dw.day) {
						dayStr += '<dd class="slt ' + _ctrClass + '" day="' + t
								+ '">' + t + '</dd>';
					} else {
						dayStr += '<dd class="' + _ctrClass + '" day="' + t
								+ '">' + t + '</dd>';
					}
				}
				...
				$days.not('.disabled').click(function() {
					...
					if (!dp.hasTime()) {
						$this.val(dp.formatDate(dp.changeDay($day.attr("day"),
								$day.attr("chMonth"))));
						closeCalendar();
					} else {
				...
				if (dp.hasTime()) {
					...
					var iMinute = parseInt(dw.minute / dp.opts.mmStep)
							* dp.opts.mmStep;
					var $minute = $(setting.minute$).val(iMinute).attr('step',
							dp.opts.mmStep).focus(function() {
								changeTmMenu("mm");
							});
					var $second = $(setting.second$).val(dp.hasSecond()
							? dw.second
							: 0).attr('step', dp.opts.ssStep).focus(function() {
								changeTmMenu("ss");
							});
			...
			$this.click(function(event) {
						...
						$(setting.okBut$).click(function() {
							...
							var date = dp.changeDay($dd.attr("day"), $dd
											.attr("chMonth"));

							if (dp.hasTime()) {
								date.setHours(parseInt($(setting.hour$).val()));
								date.setMinutes(parseInt($(setting.minute$)
										.val()));
								date.setSeconds(parseInt($(setting.second$)
										.val()));
							}
	...
	$.extend(Datepicker.prototype, {
				...
				getMaxDate : function() {
					...
					if (_count < 2) { // format:y-M、y
						...
						var _day = this._getDays(_date.getFullYear(), _date
										.getMonth()
										+ 1);
						_date.setDate(_day);
				...
				changeDay : function(day, chMonth) {
					...
					return this.changeDate(dw.year, dw.month
									+ parseInt(chMonth), day);
				},
				...
				hasTime : function() {
					return this.hasHour() || this.hasMinute()
							|| this.hasSecond();
				},
				hasDate : function() {
...
(function($) {
	$.extend($.fn, {
				...
				jBlindDown : function(options) {
					...
					return this.each(function() {
								var $this = $(this);
								var fixedPanelHeight = (op.to > 0)
										? op.to
										: $.effect.getDimensions($this[0]).height;
								$this.animate({
				...
				jSlideUp : function(options) {
					...
					return this.each(function() {
								...
								var fixedHeight = (op.to > 0)
										? op.to
										: $.effect.getDimensions($(">div",
												$this)[0]).height;
								$this.css({
											overflow : "visible",
											position : "relative"
	...
	$.extend($.fn, {
		jPanel : function(options) {
			...
			return this.each(function() {
						...
						var title = $(">h1", $panel).wrap('<div class="'
								+ op.header + '"><div class="' + op.headerC
								+ '"></div></div>');
						if (collapse)
							$("<a href=\"\"></a>").addClass(close
									? op.exp
									: op.coll).insertAfter(title);

						var header = $(">div:first", $panel);
						var footer = $('<div class="' + op.footer
								+ '"><div class="' + op.footerC
								+ '"></div></div>').appendTo($panel);

						var defaultH = $panel.attr("defH") ? $panel
								.attr("defH") : 0;
						var minH = $panel.attr("minH")
								? $panel.attr("minH")
								: 0;
						if (close)
							$content.css({
	...
	$.extend($.fn, {
								...
								$(op.selector, box).click(function() {
									...
									if (options.is(":hidden")) {
										...
										var top = box.offset().top
												+ box[0].offsetHeight - 50;
										if (top + options.height() > $(window)
												.height()
												- 20) {
				...
				combox : function() {
					...
					return this.each(function(i) {
								...
								if (ref && refUrl) {
									$this.unbind("change", _onchange).bind(
											"change", {
												ref : ref,
												refUrl : refUrl,
												$this : $this
		...
		History : {
			...
			init : function(cont, callback) {
				...
				if ($.browser.msie) {
					...
					$("body")
							.append('<iframe id="jQuery_history" style="display: none;" src="about:blank"></iframe>');
...
(function($) {
	$.printBox = function(rel) {
		...
		if ($printBox.size() == 0) {
			$printBox = $('<div id="' + _printBoxId + '"></div>')
					.appendTo("body");
		}
